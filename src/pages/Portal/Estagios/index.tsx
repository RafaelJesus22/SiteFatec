...
export const PortalInternships: React.FC = () => {
  const [files, setFiles] = useState<StorageFile[] | null>(null);
  const [listItems, setListItems] = useState<StorageFile[] | null>(null);
  ...
  const handlePressAdd = () => {
    history.push("estagios/adicionar")
  }
  ...
  const getDocuments = useCallback(async () => {
    ...
    try {
      const files = await documentService.getFilesByPath('estagios');
      setFiles(files);
    } catch (err) {
      console.log(err);
    } finally {
  ...
  return (
    <DashboardContainer hasPadding>
      <header className="portal-page_header">
        <h1 className="title">Empresas em que nossos alunos fazem estágio</h1>
      </header>
        ...
        <div className="portal-list_items">
          ...
          {listItems?.map((file) => (
            <div className="portal-list_item" key={file.name}>
              <div style={{ width: '90%' }} onClick={() => handlePressDocument(file)}>
